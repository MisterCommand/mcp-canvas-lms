...
export const GetCoursesSchema = z.object({
  enrollment_state: z
    ...
    .describe(
      "State of the course, can be 'active' (the user is currently enrolled to the course) or 'completed' (the user has completed the course), leave empty for all courses"
    )
    .optional(),
});
...
export async function getCourses(
  params: z.infer<typeof GetCoursesSchema>
): Promise<string> {
  const { enrollment_state } = params;

  // Use the pagination utility to get all courses
  const courses = await canvasRequestAllPages<
    z.infer<typeof CanvasCourseSchema>
  >("/api/v1/courses", {
    params: {
      enrollment_state,
