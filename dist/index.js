...
import { getAnnouncements, GetAnnouncementsSchema, } from "./operations/announcements.js";
import { getAssignments, GetAssignmentsSchema, } from "./operations/assignments.js";
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Get all announcements for a specific course from Canvas LMS",
                inputSchema: zodToJsonSchema(GetAnnouncementsSchema),
            },
            {
                name: "get_assignments",
                description: "Get all assignments for a specific course from Canvas LMS",
...
server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    try {
        ...
        switch (request.params.name) {
            case "get_courses":
                const coursesResponse = await getCourses(request.params.arguments);
                return {
                    content: [{ type: "text", text: coursesResponse }],
                };
            case "get_announcements":
                const announcementsResponse = await getAnnouncements(request.params.arguments);
                return {
                    content: [{ type: "text", text: announcementsResponse }],
                };
            case "get_assignments":
                const assignmentsResponse = await getAssignments(request.params.arguments);
                return {
